...
static int my_open(struct inode *inode, struct file *filp)
{
    ...
    for (i = 0; i < NPAGES; i++) {
        char *page = vmalloc_area + i * PAGE_SIZE;
        page[0] = 0xaa;
        page[1] = 0xbb;
        page[2] = 0xcc;
        page[3] = 0xdd;
        ...
        size_t size, loff_t *offset)
{
    ...
    *offset = 0;
    ...
    if (copy_to_user(user_buffer, vmalloc_area + *offset, size)) {
        ...
        size_t size, loff_t *offset)
{
    ...
    if (copy_from_user(vmalloc_area + *offset, user_buffer, size)) {
...
static int my_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    for (i = 0; i < length / PAGE_SIZE; i++) {
        pfn = vmalloc_to_pfn(vmalloc_area_ptr + i * PAGE_SIZE);
        ret = vm_insert_page(vma, start + i * PAGE_SIZE, pfn_to_page(pfn));
        if (ret < 0) {
...
static int my_seq_show(struct seq_file *seq, void *v)
{
    ...
    /* Write only the total count to match mmap-test.c parsing */
    seq_printf(seq, "%lu\n", total);
    return 0;
}
