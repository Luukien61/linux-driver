...
static const struct inode_operations minfs_dir_inode_operations = {
	...
	.create = minfs_create, /*add create */
};
...
static struct inode *minfs_iget(struct super_block *s, unsigned long ino)
{
	 ...
	 */
    bh = sb_bread(s, MINFS_INODE_BLOCK); /* Khối 1 chứa các inode */
    if (!bh) {
	...
	/* TODO 4: Lấy inode với chỉ số ino từ khối */
...
//        inode->i_fop = &simple_dir_operations;
		 ...
		 * (use inc_link) */
		 inc_nlink(inode); /* Đặt i_nlink thành 2 (1 cho thư mục, 1 cho mục '.') */
	}
...
static struct dentry *minfs_lookup(struct inode *dir, struct dentry *dentry, unsigned int flags)
{
    ...
    /* TODO 6: Comment line. */
    // return simple_lookup(dir, dentry, flags); /* Bình luận để dùng logic MinFS */

    // dentry->d_op = sb->s_root->d_op; /* Bình luận để tránh lỗi */
...
static struct inode *minfs_new_inode(struct inode *dir)
{
    ...
    find_first_zero_bit(&sbi->imap, MINFS_NUM_INODES);
    ...
    inode = new_inode(sb);
    if (!inode) {
        clear_bit(idx, &sbi->imap);  /* Rollback bitmap */
        return ERR_PTR(-ENOMEM);
    }

    /* Fill basic inode fields */
    inode->i_ino = idx;
...
found_slot:
    ...
    de->name[MINFS_NAME_LEN - 1] = '\0';  /* Ensure null termination */
...
static const struct super_operations minfs_ops = {
	...
	.alloc_inode	= minfs_alloc_inode,  /*  Thêm hàm alloc_inode */
    .destroy_inode	= minfs_destroy_inode, /* Thêm hàm destroy_inode */
...
/* TODO 2 Implementation for minfs_fill_super function */
...
/*
 * myfs_get_inode function - temporary function for root inode creation
 */
static struct inode *myfs_get_inode(struct super_block *sb, const struct inode *dir, umode_t mode)
{
